...
export class Image {
	 ...
	 */
	public toRLE(getRgbaAt: (x: number, y: number) => RGBAColor, colors: Map<string, number>): string {
		if (!this._rle) {
	 ...
	 * @param colors The color palette map
	 */
	private encode(getRgbaAt: (x: number, y: number) => RGBAColor, colors: Map<string, number>): string {
