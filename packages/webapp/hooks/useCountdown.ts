...
export const useCountdown = (endTime: number, onEnd: <T extends any[]>(...args: T) => void): Countdown => {
  ...
  return {
    isEnded: now >= end,
    countdownString: `${Math.floor(countdown / 3600)}h ${Math.floor((countdown % 3600) / 60)}m ${countdown % 60}s`,
    countdownStringDays:
      countdown > 172800
