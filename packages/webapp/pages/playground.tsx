import { Box, DarkMode, Heading, SimpleGrid, useBreakpointValue, VStack } from "@chakra-ui/react"
...
export default function Playground() {
  ...
  return (
    <DarkMode>
      <VStack w={"full"} h={"fit-content"} flexGrow={1} color={"chakra-body-text"} spacing={20}>
        <Menu />
        <Heading>Playground</Heading>
        <VStack w={"full"} spacing={20} alignItems={"center"} py={{ base: 4, lg: 20 }} px={{ base: 4, lg: 20 }}>
          ...
          <Box minW={{ base: "full", lg: "4xl" }} maxW={"full"}>
            ...
            >
              {generatedGnars.map((gnar) => {
                return <PlaygroundGnar key={gnar.id} gnartwork={gnar.gnartwork} playgroundGnarId={gnar.id} />
              })}
            </SimpleGrid>
          </Box>
        </VStack>
