...
export const ContractBreadcrumbs = forwardRef<ContractBreadcrumbsProps, "div">(({ contractInfo, ...props }, ref) => {
  return (
    <HStack
      ref={ref}
      wrap={"wrap"}
    ...
    >
      {getProxyAndImplementations(contractInfo).map((contractInfo, i) => (
        <HStack key={contractInfo.address}>
          <Link color={i > 0 ? "gray.500" : "white"} href={`https://etherscan.io/address/${contractInfo.address}#code`}>
            <Text>{contractInfo.name}</Text>
          </Link>
          {isProxy(contractInfo) && (
            <Badge fontSize={"2xs"} fontWeight={"regular"} colorScheme={"purple"}>
              Proxy
            </Badge>
          )}
        </HStack>
