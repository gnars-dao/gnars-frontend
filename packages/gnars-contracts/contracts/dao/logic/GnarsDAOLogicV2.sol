...
/// @title The Gnars DAO logic version 2
...
// LICENSE
// GnarsDAOLogicV2.sol is a modified version of Compound Lab's GovernorBravoDelegate.sol:
...
//
// MODIFICATIONS
// See GnarsDAOLogicV1 for initial GovernorBravoDelegate modifications.

// GnarsDAOLogicV2 adds:
...
// max quorum votes using the current Gnar supply.
...
contract GnarsDAOLogicV2 is GnarsDAOStorageV2, GnarsDAOEventsV2 {
    ...
    string public constant name = 'Gnars DAO';
     ...
     * @param timelock_ The address of the GnarsDAOExecutor
     * @param gnars_ The address of the GNAR tokens
    ...
    function initialize(
        ...
        require(address(timelock) == address(0), 'GnarsDAO::initialize: can only initialize once');
        if (msg.sender != admin) {
            revert AdminOnly();
        }
        require(timelock_ != address(0), 'GnarsDAO::initialize: invalid timelock address');
        require(gnars_ != address(0), 'GnarsDAO::initialize: invalid gnars address');
        require(
            votingPeriod_ >= MIN_VOTING_PERIOD && votingPeriod_ <= MAX_VOTING_PERIOD,
            'GnarsDAO::initialize: invalid voting period'
        );
        require(
            votingDelay_ >= MIN_VOTING_DELAY && votingDelay_ <= MAX_VOTING_DELAY,
            'GnarsDAO::initialize: invalid voting delay'
        );
        require(
            proposalThresholdBPS_ >= MIN_PROPOSAL_THRESHOLD_BPS && proposalThresholdBPS_ <= MAX_PROPOSAL_THRESHOLD_BPS,
            'GnarsDAO::initialize: invalid proposal threshold bps'
        );
        ...
        timelock = IGnarsDAOExecutor(timelock_);
        gnars = GnarsTokenLike(gnars_);
        vetoer = vetoer_;
    ...
    function propose(
        ...
        temp.totalSupply = gnars.totalSupply();

        temp.proposalThreshold = bps2Uint(proposalThresholdBPS, temp.totalSupply);

        require(
            gnars.getPriorVotes(msg.sender, block.number - 1) > temp.proposalThreshold,
        ...
        require(
            ...
            'GnarsDAO::propose: proposal function information arity mismatch'
        );
        require(targets.length != 0, 'GnarsDAO::propose: must provide actions');
        require(targets.length <= proposalMaxOperations, 'GnarsDAO::propose: too many actions');
        ...
        if (temp.latestProposalId != 0) {
            ...
            require(
                ...
                'GnarsDAO::propose: one live proposal per proposer, found an already active proposal'
            );
            require(
                proposersLatestProposalState != ProposalState.Pending,
                'GnarsDAO::propose: one live proposal per proposer, found an already pending proposal'
            );
        }
    ...
    function queue(uint256 proposalId) external {
        require(
            ...
            'GnarsDAO::queue: proposal can only be queued if it is succeeded'
        );
    ...
    function queueOrRevertInternal(
        ...
        require(
            ...
            'GnarsDAO::queueOrRevertInternal: identical proposal action already queued at eta'
        );
    ...
    function execute(uint256 proposalId) external {
        require(
            ...
            'GnarsDAO::execute: proposal can only be executed if it is queued'
        );
        Proposal storage proposal = _proposals[proposalId];
    ...
    function cancel(uint256 proposalId) external {
        ...
        require(
            msg.sender == proposal.proposer ||
                gnars.getPriorVotes(proposal.proposer, block.number - 1) <= proposal.proposalThreshold,
            'GnarsDAO::cancel: proposer above threshold'
        );
    ...
    function state(uint256 proposalId) public view returns (ProposalState) {
        require(proposalCount >= proposalId, 'GnarsDAO::state: invalid proposal id');
        Proposal storage proposal = _proposals[proposalId];
        if (proposal.vetoed) {
    ...
    function castVoteBySig(
        ...
        require(signatory != address(0), 'GnarsDAO::castVoteBySig: invalid signature');
    ...
    function castVoteInternal(
        ...
        require(state(proposalId) == ProposalState.Active, 'GnarsDAO::castVoteInternal: voting is closed');
        require(support <= 2, 'GnarsDAO::castVoteInternal: invalid vote type');
        Proposal storage proposal = _proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, 'GnarsDAO::castVoteInternal: voter already voted');

        /// @notice: Unlike GovernerBravo, votes are considered from the block the proposal was created in order to normalize quorumVotes and proposalThreshold metrics
        uint96 votes = gnars.getPriorVotes(voter, proposalCreationBlock(proposal));
    ...
    function _setVotingDelay(uint256 newVotingDelay) external {
        ...
        require(
            ...
            'GnarsDAO::_setVotingDelay: invalid voting delay'
        );
    ...
    function _setVotingPeriod(uint256 newVotingPeriod) external {
        ...
        require(
            ...
            'GnarsDAO::_setVotingPeriod: invalid voting period'
        );
    ...
    function _setProposalThresholdBPS(uint256 newProposalThresholdBPS) external {
        ...
        require(
            ...
            'GnarsDAO::_setProposalThreshold: invalid proposal threshold bps'
        );
    ...
    function _setMinQuorumVotesBPS(uint16 newMinQuorumVotesBPS) external {
        ...
        require(
            ...
            'GnarsDAO::_setMinQuorumVotesBPS: invalid min quorum votes bps'
        );
        require(
            newMinQuorumVotesBPS <= params.maxQuorumVotesBPS,
            'GnarsDAO::_setMinQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setMaxQuorumVotesBPS(uint16 newMaxQuorumVotesBPS) external {
        ...
        require(
            ...
            'GnarsDAO::_setMaxQuorumVotesBPS: invalid max quorum votes bps'
        );
        require(
            params.minQuorumVotesBPS <= newMaxQuorumVotesBPS,
            'GnarsDAO::_setMaxQuorumVotesBPS: min quorum votes bps greater than max'
        );
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        // Check caller = admin
        require(msg.sender == admin, 'GnarsDAO::_setPendingAdmin: admin only');
    ...
    function _acceptAdmin() external {
        ...
        require(msg.sender == pendingAdmin && msg.sender != address(0), 'GnarsDAO::_acceptAdmin: pending admin only');
    ...
    function _burnVetoPower() public {
        ...
        require(msg.sender == vetoer, 'GnarsDAO::_burnVetoPower: vetoer only');
    ...
    /**
     * @notice Current proposal threshold using Gnar Total Supply
     * Differs from `GovernerBravo` which uses fixed amount
     */
    function proposalThreshold() public view returns (uint256) {
        return bps2Uint(proposalThresholdBPS, gnars.totalSupply());
    }
     ...
     * @param totalSupply The total supply of Gnars at the time of proposal creation
    ...
    function getDynamicQuorumParamsAt(uint256 blockNumber_) public view returns (DynamicQuorumParams memory) {
        uint32 blockNumber = safe32(blockNumber_, 'GnarsDAO::getDynamicQuorumParamsAt: block number exceeds 32 bits');
    ...
    /**
     * @notice Current min quorum votes using Gnar total supply
     */
    function minQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).minQuorumVotesBPS, gnars.totalSupply());
    }

    /**
     * @notice Current max quorum votes using Gnar total supply
     */
    function maxQuorumVotes() public view returns (uint256) {
        return bps2Uint(getDynamicQuorumParamsAt(block.number).maxQuorumVotesBPS, gnars.totalSupply());
    }
